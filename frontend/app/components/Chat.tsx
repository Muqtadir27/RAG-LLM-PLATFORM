...
export default function Chat({ selectedDoc, messages, setMessages }: ChatProps) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const finalMessages = [...updated, { role: 'assistant' as const, content: answer, sources: data.sources }]
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
      ...
      {/* MESSAGES */}
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', display: 'flex', flexDirection: 'column', padding: '20px 28px', gap: 16, justifyContent: isEmpty ? 'center' : 'flex-start', alignItems: isEmpty ? 'center' : 'stretch' }}>
      ...
      <div style={{ flexShrink: 0, padding: '12px 24px 16px', background: 'rgba(0,4,12,0.95)', borderTop: '1px solid rgba(0,212,255,0.08)', backdropFilter: 'blur(20px)' }}>
        ...
        >
          ...
          <button
            ...
            style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', width: 32, height: 32, borderRadius: 7, background: question.trim() && !loading ? 'linear-gradient(135deg,rgba(0,212,255,0.25),rgba(0,80,200,0.25))' : 'rgba(0,212,255,0.04)', border: `1px solid ${question.trim() && !loading ? 'rgba(0,212,255,0.4)' : 'rgba(0,212,255,0.1)'}`, color: question.trim() && !loading ? '#00d4ff' : 'rgba(0,212,255,0.2)', cursor: question.trim() && !loading ? 'pointer' : 'not-allowed', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
          >▶</button>
        </div>
